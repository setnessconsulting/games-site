// GAME-365 / PS-HOST: the Planetary Survey host contract.
//
// The story exists to prepare the host for a game WITHOUT promoting an unfinished build.
...
//
// Both are asserted in both environment states, because a check that only ever runs in
// one of them cannot distinguish "safe by design" from "safe by luck". The pure catalog
// invariant tests run in both states too, since a validator that only rejects a bad shape
// on the preview deployment is not an invariant.
...
import PlanetarySurveyLauncher from "../src/pages/planetary-survey/index.astro";
import PlanetarySurveyPlay from "../src/pages/planetary-survey/play.astro";

const PREVIEW_VERSION = "0.1.0-ps-host-evidence.1";

const planetarySurvey = (): GameEntry => {
...
async function importWithPreview(version: string) {
  vi.stubEnv("PLANETARY_SURVEY_PREVIEW_VERSION", version);
  vi.resetModules();
  return await import("../src/data/games");
}

describe("planetary survey host identity", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.resetModules();
  });
  ...
  it("is in the catalog as coming-soon with an ordered route and real controls", () => {
    const game = planetarySurvey();

    expect(game.status).toBe("coming-soon");
    expect(game.route).toBe("/planetary-survey/");
    expect(getGamePlayRoute(game)).toBe("/planetary-survey/play/");
    expect(game.title).toBe("Planetary Survey");
...
describe("production selects no Planetary Survey release", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.resetModules();
  });
  ...
  it("declares no production release and no preview pointer", () => {
    ...
    // There is no PLANETARY_SURVEY_PRODUCTION_VERSION constant either: a missing
    // constant fails to compile, which is a stronger guarantee than a placeholder.
  ...
  it("renders the launcher with no pointer and honest unavailable copy", async () => {
    ...
    const html = await container.renderToString(PlanetarySurveyLauncher, { partial: false });

    expect(html).toContain('data-slot="planetary-survey-host-status"');
    ...
    expect(html).toContain("No Planetary Survey build is selected here.");
    // The promoted-play affordance must be absent, not merely relabelled.
    expect(html).not.toContain(">Play Planetary Survey");
  });

  it("renders the play route as the shared unavailable panel, never a frame", async () => {
    const container = await AstroContainer.create();
    const html = await container.renderToString(PlanetarySurveyPlay, { partial: false });

    expect(html).toContain('class="not-ready-panel"');
    expect(html).toContain("The survey probe is still being prepared.");
...
describe("a supplied preview pointer is a candidate, not a promotion", () => {
  ...
  it("still declares no production release while the candidate is configured", async () => {
    ...
    const game = getPreviewGame(PLANETARY_SURVEY_SLUG)!;
  ...
  it("distinguishes the preview pointer from the production pointer", async () => {
    ...
    const game = games.getGame(PLANETARY_SURVEY_SLUG)!;
    ...
    expect(games.getGamePreviewSource(game)).toEqual({
      assetBase: `/game-assets/${PLANETARY_SURVEY_SLUG}/${PREVIEW_VERSION}`,
  ...
  it("renders the launcher pointer and a clearly-labelled candidate link", async () => {
    ...
    // Imported dynamically on purpose. A top-level static import would already have
    // been evaluated with the production environment, so the component would keep
    // rendering the no-pointer state and the test would pass for the wrong reason.
    const { default: Launcher } = await import("../src/pages/planetary-survey/index.astro");
    const container = await AstroContainer.create();
    ...
    expect(html).toContain(
      `data-preview-asset-base="/game-assets/${PLANETARY_SURVEY_SLUG}/${PREVIEW_VERSION}"`
    );
  ...
  it("renders the play route against the exact candidate asset base", async () => {
    await importWithPreview(PREVIEW_VERSION);
    const { default: Play } = await import("../src/pages/planetary-survey/play.astro");
    const container = await AstroContainer.create();
    const html = await container.renderToString(Play, { partial: false });

    expect(html).toContain(
      `src="/game-assets/${PLANETARY_SURVEY_SLUG}/${PREVIEW_VERSION}/index.html"`
    );
...
describe("catalog invariants that keep the pointers distinct", () => {
  const base = (): GameEntry => ({ ...planetarySurvey() });
...
describe("asset serving requires an exact approved pointer", () => {
  it("serves nothing for an unapproved candidate version", () => {
    expect(isApprovedRelease(PLANETARY_SURVEY_SLUG, PREVIEW_VERSION, false)).toBe(false);
    expect(isApprovedRelease(PLANETARY_SURVEY_SLUG, "0.1.0", false)).toBe(false);
  });
  ...
  it("cannot approve the game through the promoted-release fallback", () => {
    // The catalog fallback only approves a playable game's selected release. Planetary
    // Survey is not playable and selects nothing, so no catalog state can approve it —
    // only an explicit deployment pointer can.
    const promoted = games
      .filter((game) => game.slug === PLANETARY_SURVEY_SLUG)
...
describe("route validation covers the preview-gated route", () => {
  // Page files must be supplied explicitly: with no discovered routes the registry
  ...
  it("flags previewEnabled when the play page is missing", async () => {
    ...
    const entry: GameEntry = { ...planetarySurvey(), previewEnabled: true };
